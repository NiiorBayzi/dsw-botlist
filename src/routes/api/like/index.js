...
route.patch("/:id", auth, async (req, res) => {
  ...
  if (!webhook) 
    webhook = await channel.createWebhook('Dark Side World')
  await webhook.send(`**( <:dsw_wolfDiamond:998885240649023488> ) › <@${req.user.id}>** (${userProfile.tag}) votou no bot <@${req.params.id}>.`);
